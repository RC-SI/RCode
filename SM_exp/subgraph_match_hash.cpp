...
int CNRange(int *vec1, int l1, int *vec2, int l2, int *cn, std::vector<int>& hashes) {
    ...
    if (l1 != 0 && l2 != 0) {
        ...
        else if (std::max(hashes[v1s], hashes[v2s])<=std::min(hashes[v1e], hashes[v2e])) {
            int b1 = 0;
            int f1 = l1;
            int b2 = 0;
            int f2 = l2;
            if (v1s<v2s)
                b1 = binarySearch(v2s, vec1, b1, f1);
            else if (v2s>v1s)
                b2 = binarySearch(v1s, vec2, b2, f2);
            if (v1e>v2e)
                f1 = binarySearch(v2e+1, vec1, b1, f1);
            else if (v1e<v2e)
                f2 = binarySearch(v1e+1, vec2, b2, f2);
            while (b1 != f1 && b2 != f2) {
                if (vec1[b1] == vec2[b2]) {
                    cn[size_c++] = vec1[b1];
                    b1++;
